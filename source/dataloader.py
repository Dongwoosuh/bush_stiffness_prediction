...
__all__ = ["BushDataset", "VEPDataset", "InferenceVEPDataset","IntegrateDataset"]
# In[3. Data setting] #############################################################################################
...
class IntegrateDataset(VEPDataset):
    def __init__(self, output_path:str, test_key:str, exclude_keys:list=None):
        ...
        train_inputs = np.array([np.asarray(i, dtype=np.float32) for i in train_inputs])
        train_inputs[:, 14:] = np.log1p(train_inputs[:, 14:])
        
        
        # 음수인 부분은 부호를 바꿔 양수로 만든 뒤 로그를 씌우고, 다시 음수로 복원
        train_outputs = np.array([np.asarray(o, dtype=np.float32) for o in train_outputs])
        sign = np.sign(train_outputs)
        train_outputs = np.log1p(np.abs(train_outputs))
        train_outputs = train_outputs * sign
        # train_outputs = np.log1p(train_outputs)
        ...
        if len(test_inputs) == 0:
            ...
            # test_inputs = np.hstack((test_inputs, x_disp_.reshape(-1, 1), z_disp_.reshape(-1, 1), theta_x_.reshape(-1,1)))
            test_inputs = np.array([np.asarray(i, dtype=np.float32) for i in test_inputs])
            test_inputs[:, 14:] = np.log1p(test_inputs[:, 14:])
            test_outputs = np.array([np.asarray(o, dtype=np.float32) for o in test_outputs])
            # test_outputs = np.log1p(test_outputs)
    ...
    def get_datasets(self):
        ...
        train_input_shape = train_input[:, :14]
        train_input_linear = train_input[:,14:].flatten().reshape(-1,1)
        
        val_input_shape = val_input[:, :14]
        val_input_linear = val_input[:,14:].flatten().reshape(-1,1)
        ...
        self.input_scaler_linear.fit(train_input_linear)
        train_input_linear = self.input_scaler_linear.transform(train_input_linear).reshape(train_input[:,14:].shape)
        val_input_linear = self.input_scaler_linear.transform(val_input_linear).reshape(val_input[:,14:].shape)
        ...
        train_output = train_output.reshape(-1, 1, 31, 31)
        val_output = val_output.reshape(-1, 1, 31, 31)
        # Create datasets and data loaders
        train_dataset = BushDataset(train_input, train_output)
        val_dataset = BushDataset(val_input, val_output)
        # test_dataset = BushDataset(self.np_test_input, self.np_test_output)
